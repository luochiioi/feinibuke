...
//                                  totalAdminDeleteUser, totalPhotoPurged, totalPhotoPurgeFailed }
//
...
//   purgePhoto:     boolean —— 是否触发物理删图
//   purgeError:     string  —— 物理删失败的错误（成功时为空字符串）
...
function summarizeAuditLogs(rows) {
  ...
  ;(rows || []).forEach(row => {
    ...
    if (row.purgePhoto === true) {
      if (row.purgeError && String(row.purgeError).length > 0) {
        summary.totalPhotoPurgeFailed += 1
      } else {
        summary.totalPhotoPurged += 1
      }
    }
  })
  return summary
